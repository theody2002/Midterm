...
class Product
{
	...
	{
		$sql = "UPDATE tbl_product SET prod_name=:prod_name,prod_description=:prod_description,type_id=:type_id WHERE prod_id=:prod_id";

		$q = $this->conn->prepare($sql);
		$q->execute(array(':prod_name' => $pname, ':prod_description' => $desc, ':type_id' => $ptype, ':prod_id' => $id));
		return true;
	}
	function update_product_image($prod_id, $prod_image)
{
    $sql = "UPDATE tbl_product SET prod_image=:prod_image WHERE prod_id=:prod_id";
    $q = $this->conn->prepare($sql);
    $q->execute(['prod_image' => $prod_image, 'prod_id' => $prod_id]);
    return true;
}
